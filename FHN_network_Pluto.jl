...
# ╔═╡ 2245cabc-bb64-11ef-3c07-e1aa3352f8d9
begin
	using Plots, DynamicalSystems, DelimitedFiles, GraphPlot, Graphs, Statistics, AbstractFFTs, Random, Measures, LaTeXStrings, Combinatorics
	Random.seed!(1234)
	Threads.nthreads()
	
end

# ╔═╡ b9661f11-864f-402c-b5ec-98b389ffdc2e
begin
    n = 10
    ρ = 0
    k = 1
    g = watts_strogatz(n, 2 * k, ρ, is_directed=false, seed=1234)
    am = adjacency_matrix(g)
    Am = Matrix(adjacency_matrix(g))
    Graph = gplot(g, layout=circular_layout, nodefillc="red", edgestrokec=colorant"blue")
end


# ╔═╡ a3e44b07-3551-4138-95e7-6c62a5597fd0

begin
    Adj_Matrix = heatmap(Am, framestyle=:box, grid=false, color=:gist_stern,
        xlabel=L"j", ylabel=L"i", cbar=false,
        xlim=(1, n), ylim=(1, n),
        guidefont=font(15, "Computer Modern"),
        tickfont=font(14, "Computer Modern"),
        titlefont=font(14, "Computer Modern"),
        margin=8.0mm, dpi=500)
end

# ╔═╡ 1e25aebe-f18c-4eac-9bd2-c50ae63e9d69
function FHN_Base!(du, u, p, t)
    a, b, ϵ, fpa, gamma = p
    for i = 1:n
        du[1, i] = (-u[1, i] * (u[1, i] - a) * (u[1, i] - 1)) - (gamma * u[2, i]) + (ϵ / (2)) * sum(j -> (Am[i, j] * ((u[1, j] - u[1, i]))), 1:n) 
        du[2, i] = b * ((gamma * u[1, i]) - fpa * u[2, i])
    end
end

# ╔═╡ 16a3f583-7d0f-40b9-ae29-3e39e8c2c425
begin
    a = -0.02 #-0.025
    b = 0.0065
    fpa = 0.002  #0.022
...
# ╔═╡ 4cd0cfc6-9e30-4b39-9fc1-1a645431f2e1
begin
	
	    Random.seed!(1234)
	    u0 = [1 * (rand(1, n)); 1 * (rand(1, n))]
	    p = [a, b, ϵ, fpa, gamma]
	    lsl = ContinuousDynamicalSystem(FHN_Base!, u0, p)
	    trs, T = trajectory(lsl, 1000.0; Δt=0.01, Ttr=100)
	
end

# ╔═╡ 57702835-deee-4fb5-ac6c-01bc77e7e9e5
heatmap(Matrix(trs[:, 1:2:2*n]))
...
manifest_format = "2.0"
project_hash = "179f5723f4432cb4147f876c800f4676c82dec88"
...
git-tree-sha1 = "89f8e065d97f943658c51ccbc48b08e8e435eb5e"

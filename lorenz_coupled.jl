...
# ╔═╡ 0cf759ef-ee20-469a-9a8f-cf602362f30d
using Plots, DynamicalSystems, LaTeXStrings, DifferentialEquations, Peaks, Measures

# ╔═╡ f0337384-bb5a-11ef-2e84-d5d5f1fd18e6
...
# ╔═╡ 3895e351-bfbd-4a59-9b8f-88a91946d8bc
begin
u0 = [1.0, 1.0, 1.0]  # Initial values for x, y, z
u01 = [1.0001, 1.0, 1.0]  # Initial values for x, y, z
p = (10.0, 28.0, 8.0 / 3.0)

	
    diffeq = (alg=Tsit5(), abstol=1e-9, reltol=1e-9)
    ds = ContinuousDynamicalSystem(lorenz!, u0, p; diffeq)

	ds1 = ContinuousDynamicalSystem(lorenz!, u01, p; diffeq)
end

# ╔═╡ 586957dc-a3d1-47ed-a25b-a3edae091b07
begin
	@time tr, T = trajectory(ds, 50.0, Dt=0.01, Ttr=0)
	@time tr1, T1 = trajectory(ds1, 50.0, Dt=0.01, Ttr=0)
end

# ╔═╡ c9e14f05-908f-45a1-8e02-44bd30edc7c6
begin
	    p1 = plot(tr[1:1000, 1], lc=:black, label=L"System 1")
	    p1 = plot!(tr1[1:1000, 1], xlab=L"T", label=L"System 2",
	        ylabel=L"x", lc=:red,
	        guidefont=font(20, "Computer Modern"), legendfont=font(14, "Computer Modern"),
...
# ╔═╡ f29a4c49-7a04-4b03-bb3f-eb3351d832e2
begin
	    p2 = plot(tr[1:5000, 1],tr[1:5000, 2], lc=:black, label=L"System 1")
	    p3 = plot!(tr1[1:5000, 1], tr1[1:5000, 2], xlab=L"x", label=L"System 2",
	        ylabel=L"y", lc=:red,
	        guidefont=font(20, "Computer Modern"), legendfont=font(14, "Computer Modern"),
	        tickfont=font(16, "Computer Modern"),
	        dpi=300,
	        framestyle=:box,
	        # xlim=(-0.7500, 1.5), line=(:dot, 3),
	        # ylim=(-0.25, 0.25),
	        margin=0mm)
end
...
manifest_format = "2.0"
project_hash = "c05639364330e483499269ce7c467bcf261f2830"
